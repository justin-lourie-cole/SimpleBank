...
contract('SimpleBank - basic initialization', accounts => {
  const alice = accounts[1]
  const bob = accounts[2]
  const charlie = accounts[3]
  const dave = accounts[4]

  it('should rewards 3 first clients with 10 balance', async () => {
    const bank = await SimpleBank.deployed()

    await bank.enroll({ from: alice })
    const aliceBalance = await bank.balance({ from: alice })
    assert.equal(aliceBalance, reward, 'inital balance is incorrect')

    await bank.enroll({ from: bob })
    const bobBalance = await bank.balance({ from: bob })
    assert.equal(bobBalance, reward, 'inital balance is incorrect')

    await bank.enroll({ from: charlie })
    const charlieBalance = await bank.balance({ from: charlie })
    assert.equal(charlieBalance, reward, 'inital balance is incorrect')

    await bank.enroll({ from: dave })
    const daveBalance = await bank.balance({ from: dave })
    assert.equal(daveBalance, 0, 'inital balance is incorrect')

    const despositsBalance = await bank.depositsBalance()
    assert.equal(
      despositsBalance,
      initialDepositsBalance,
  ...
  it('should deposit correct amount', async () => {
    ...
    assert.equal(
      despositsBalance,
      initialDepositsBalance + deposit,
      'banks deposits balance should be increased'
    )

    const expectedEventResult = { accountAddress: alice, amount: deposit }
    assert.equal(
      receipt.logs[0].args.accountAddress,
...
contract('SimpleBank - proper withdrawal', accounts => {
  const alice = accounts[1]
...
contract('SimpleBank - incorrect withdrawal', accounts => {
  const alice = accounts[1]
...
contract('SimpleBank - fallback works', accounts => {
  const alice = accounts[1]
  ...
  it('should revert ether sent to this contract through fallback', async () => {
    ...
    try {
      ...
      assert(error, 'Error: VM Exception while processing transaction: revert')
    }
